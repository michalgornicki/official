...
const Counters = () => {
  ...
  const counter = () => {
    ...
    var intervSpeed = setInterval(function () {
      if (i < 4) document.getElementById("counter1").innerHTML = ++i;
      else clearInterval(intervSpeed);
    }, 15);

    var intervPower = setInterval(function () {
      if (i < 20) document.getElementById("counter2").innerHTML = ++i;
      else clearInterval(intervPower);
    }, 5);

    var intervDispl = setInterval(function () {
      if (j < 2167) document.getElementById("counter3").innerHTML = ++j;
