...
const Profile = () => {
  ...
  const mouseMove = (e) => {
    ...
    setMousePosX(X);
    setMousePosY(Y);
    document.getElementsByClassName("body")[0].style.filter = "brightness(" + mousePosY/400 + 0.2 + ") sepia(" + 8/mousePosX + ")";
    document.getElementsByClassName("profile-photo")[0].style.backgroundPosition =  mousePosX/20 + "%";

    document.getElementById("container1").style.transform = "rotateX(" + (mousePosY/30 - 10).toFixed(2) + "deg) rotateY(" + (mousePosX/30 - 10).toFixed(2) + "deg)" ;
  };
